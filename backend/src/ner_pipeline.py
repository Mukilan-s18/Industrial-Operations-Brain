...
class NERPipeline:
    ...
    def extract_entities(
        ...
        for match in date_pattern.finditer(text):
            ...
            overlap = any(
                e.start_char < span_end and e.end_char > span_start for e in spacy_ents
            )
            if not overlap:
                # Add date
                extracted.append(
        ...
        for val_start, val_end, val_text in param_values:
            ...
            for ext in extracted:
                if ext.label == "PARAMETER":
                    # Calculate distance
                    dist = min(
                        abs(ext.span_start - val_end), abs(val_start - ext.span_end)
                    )
    ...
    def evaluate_accuracy(self, labeled_sentences_path: str = None) -> Dict[str, Any]:
        ...
        if labeled_sentences_path is None:
            labeled_sentences_path = os.path.join(
                os.path.dirname(__file__), "..", "..", "data", "labeled_sentences.json"
            )
