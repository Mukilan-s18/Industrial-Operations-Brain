...
def execute_action(state: RCAState):
    """Day 8: Closed-Loop Agentic Action Execution (SAP Mock) using LLM Decision"""
    ans = state.get("final_answer", "")
    llm = get_llm()
    prompt = f"""Analyze the following final RCA answer and determine if a maintenance work order should be automatically generated.
    If the answer recommends maintenance, creates a draft, or highlights a critical failure, you should create a work order.
    
    Respond ONLY with a valid JSON object matching this schema:
    {{
        "should_create_work_order": boolean,
        "equipment_id": "string (extract the equipment tag, e.g. P-101, or 'Unknown')",
        "priority": "string (High, Medium, Low)",
        "reason": "string (short 1-sentence description)"
    }}
    ...
    try:
        ...
        if decision.get("should_create_work_order"):
            mock_id = f"SAP-WO-{int(time.time())}"
            db_path = os.path.join(os.path.dirname(__file__), "..", "..", "mock_sap.db")
            conn = sqlite3.connect(db_path)
            c = conn.cursor()
            c.execute(
                "CREATE TABLE IF NOT EXISTS work_orders (id TEXT, equipment_id TEXT, priority TEXT, description TEXT)"
            )

            # Check if old table schema exists and upgrade if necessary, but for mock just try inserting
            try:
                c.execute(
                    "INSERT INTO work_orders (id, equipment_id, priority, description) VALUES (?, ?, ?, ?)",
                    (
                        mock_id,
                        decision.get("equipment_id", "Unknown"),
                        decision.get("priority", "Medium"),
                        decision.get("reason", "Generated by AI"),
                    ),
                )
            except sqlite3.OperationalError:
                # Fallback to old schema if table already existed without new columns
                c.execute(
                    "INSERT INTO work_orders VALUES (?, ?)",
                    (
                        mock_id,
                        f"[{decision.get('priority')}] {decision.get('equipment_id')} - {decision.get('reason')}",
                    ),
                )
            ...
            return {
                ...
                "action_result": f"Successfully created {decision.get('priority')} priority Work Order {mock_id} for {decision.get('equipment_id')}.",
                "status": f"Agent executed action: Created Work Order {mock_id}",
            }
    except Exception as e:
        logger.error("Error executing action via LLM", error=str(e))
