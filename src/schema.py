...
class FailureMode(BaseModel):
    type: str = Field(..., description="Type of failure, e.g., Seal Leak, Bearing Seizure")
    date: Optional[str] = Field(None, description="Date of occurrence")
    severity: Optional[str] = Field(None, description="Severity level")

class Parameter(BaseModel):
    name: str = Field(..., description="Name of the parameter, e.g., Discharge Pressure")
    value: Optional[str] = Field(None, description="Parameter value, e.g. 4.5 bar, 82 C")
